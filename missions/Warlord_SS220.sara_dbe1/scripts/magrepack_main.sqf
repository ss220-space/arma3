outlw_MR_createDialog =
{
	...
	((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 22170) lbAdd "Все боеприпасы";
	...
	{
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9002) ctrlSetStructuredText parseText "Debug Mode: <t align='right'>Да</t>";
	}
	else
	{
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9002) ctrlSetStructuredText parseText "Debug Mode: <t align='right'>Нет</t>";
	};

	if (outlw_MR_doHideFull) then
	{
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9004) ctrlSetStructuredText parseText "Полные: <t align='right'>Нет</t>";
	}
	else
	{
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9004) ctrlSetStructuredText parseText "Полные: <t align='right'>Нет</t>";
	...
	((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9005) ctrlSetStructuredText parseText "О аддоне";
...
outlw_MR_populateMagListBox =
{
	...
	for "_n" from 0 to ((count _magTypes) - 1) do
	{
		...
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 1500) lbSetPicture [_n, format ["Images\bulletCount\%1.paa", round((_magAmmoCounts select _n)/(_magAmmoCaps select _n)*30)]];
		...
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 1500) lnbSetPicture [[_n, 1], format ["Images\bulletCount\%1.paa", round((_magAmmoCounts select _n)/(_magAmmoCaps select _n)*30)]];
...
outlw_MR_block =
{
	...
	switch (true) do
	{
		case (outlw_MR_sourceType != ""): {((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 2215) ctrlSetToolTip "Магазин уже указан!";};
		case (outlw_MR_dragCount == 0): {((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 2215) ctrlSetToolTip "Магазин не может быть пуст!";};
		case (outlw_MR_dragCap < 100 && (outlw_MR_targetType != "" && outlw_MR_targetCap >= 100)):
		{
			((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 2215) ctrlSetToolTip "Для этого требуется лента а не отдельные пули!";
		};
	...
	switch (true) do
	{
		case (outlw_MR_targetType != ""): {((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 2216) ctrlSetToolTip "Магазин уже указан!";};
		case (outlw_MR_dragCount == outlw_MR_dragCap && outlw_MR_dragCap != 1): {((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 2216) ctrlSetToolTip "Магазин уже заполнен!";};
		case (outlw_MR_dragCap >= 100 && (outlw_MR_sourceType != "" && outlw_MR_sourceCap < 100)):
		{
			((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 2216) ctrlSetToolTip "Для этого магазина требуется лента а не отдельные пули!";
		};
...
outlw_MR_sourceEnabled =
{
	...
	else
	{
		...
		{
			((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 1600) ctrlSetText "Перепаковать";
...
outlw_MR_targetEnabled =
{
	...
	else
	{
		...
		{
			((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 1601) ctrlSetText "Перепаковать";
...
outlw_MR_showFullSwitch =
{
	if (outlw_MR_doHideFull) then
	{
		...
		((uiNamespace getVariable "outlw_MR_Dialog_Main") displayCtrl 9004) ctrlSetStructuredText parseText "Полные: <t align='right'>Да</t>";
	}
	else
	{
...
outlw_MR_onDialogDestroy =
{
	...
	if (outlw_MR_debugMode) then
	{
		...
		for "_n" from 0 to ((count _sTCA) - 1) do
		{
			...
			_toAdd = ((getText (configFile >> "cfgMagazines" >> _snTCA select 0 >> "DisplayName")) + "<br/>[" + str(_snTCA select 1) + "] >> [0]<br/>Патроны: <t color='#FC1010'>-" + str(_snTCA select 2) + "</t>");
			...
			for "_a" from 0 to ((count _eTCA) - 1) do
			{
				...
				if ((_snTCA select 0) == (_enTCA select 0)) then
				{
					...
					switch (true) do
					{
						case (_dif > 0): {_toAdd = (_toAdd + "Патроны: <t color='#15E612'>+" + str(_dif) + "</t>")};
						case (_dif < 0): {_toAdd = (_toAdd + "Патроны: <t color='#FC1010'>" + str(_dif) + "</t>")};
						default {_toAdd = (_toAdd + "Патроны: без изменений")};
					};
				};
			};
		...
		for "_n" from 0 to ((count _eTCA) - 1) do
		{
			...
			_toAdd = ((getText (configFile >> "cfgMagazines" >> _enTCA select 0 >> "DisplayName")) + "<br/>[0] >> [" + str(_enTCA select 1) + "]<br/>Патроны: <t color='#15E612'>+" + str(_enTCA select 2) + "</t><br/><br/>");
